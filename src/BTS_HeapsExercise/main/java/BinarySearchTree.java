package BTS_HeapsExercise.main.java;
...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    public E floor(E element) {
        ...
        Node<E> current = this.root;
        Node<E> nearestSmallest = null;
        while (current != null) {
            if (isGreater(element, current)) {
                current = current.getRight();
                nearestSmallest = current;
            } else if (isLess(element, current)) {
                current = current.getLeft();
            } else {
                Node<E> left = current.getLeft();

                if (left != null && nearestSmallest != null) {
                    nearestSmallest = isGreater(left.getValue(), nearestSmallest) ? left : nearestSmallest;
                } else if (nearestSmallest == null) {
                    nearestSmallest = left;
                }
                break;

            }
        }
        return nearestSmallest == null ? null : nearestSmallest.getValue();
    }
}
