...
public class BinaryTree {
    ...
    private void traverseTree(BinaryTree binaryTree, int offset, int level, Map<Integer, Pair<Integer, Integer>> offsetToLevel) {
        if (binaryTree == null) {
            return;
        }
        Pair<Integer, Integer> currentValueLevel = offsetToLevel.get(offset);
        if (currentValueLevel == null || level < currentValueLevel.getValue()) {
            offsetToLevel.put(offset, new Pair<>(binaryTree.value, level));
        }
        traverseTree(binaryTree.left, offset - 1, level + 1, offsetToLevel);
        traverseTree(binaryTree.right, offset + 1, level + 1, offsetToLevel);
    }
}
