package RedBlackTreesAA_Exercise.main.java;
...
public class RedBlackTree<Key extends Comparable<Key>, Value> {
    ...
    private Node moveRedLeft(Node node) {
        flipColors(node);
        if (this.isRed(node)) {
            node = this.rotateLeft(node.right);
            flipColors(node);
            node.size = this.size(node.left) + this.size(node.right) + 1;
        }
        return node;
    ...
    private Key select(Node node, int rank) {
        ...
        if (this.size(node.left) < rank) {
            return this.select(node.right, rank - this.size(node.left));
